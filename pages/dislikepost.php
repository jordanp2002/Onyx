...
if (isset($_SESSION['username']) && isset($_POST['thread_id'])) {
    ...
    if ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($count > 0) {
            if($resultRow['thread_dislike'] == 1){
                $actionQuery = mysqli_prepare($connection, "UPDATE likes SET thread_dislike = 0 WHERE account_id = ? AND thread_id = ?");
                mysqli_stmt_bind_param($actionQuery, "ii", $accountId, $threadId);
                mysqli_stmt_execute($actionQuery);
                $response = 'dislike';
            } else {
                $actionQuery = mysqli_prepare($connection, "UPDATE likes SET thread_dislike = 1 WHERE account_id = ? AND thread_id = ?");
                mysqli_stmt_bind_param($actionQuery, "ii", $accountId, $threadId);
                mysqli_stmt_execute($actionQuery);
                $response = 'undislike';
            }
        } else {
            $actionQuery = mysqli_prepare($connection, "INSERT INTO likes (account_id, thread_id, thread_like, thread_dislike) VALUES (?, ?, 0, 1)");
            mysqli_stmt_bind_param($actionQuery, "ii", $accountId, $threadId);
            $response ='undislike';
        }
        mysqli_stmt_execute($actionQuery);
